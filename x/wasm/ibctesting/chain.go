...
type TestChain struct {
	...
	Signers map[string]octypes.PrivValidator
...
func NewTestChain(t *testing.T, coord *Coordinator, chainID string, opts ...wasm.Option) *TestChain {
	...
	var (
		...
		validators         = make([]*octypes.Validator, 0, validatorsPerChain)
	...
	for i := 0; i < validatorsPerChain; i++ {
		...
		validators = append(validators, octypes.NewValidator(pubKey, 1))
...
func NewTestChainWithValSet(t *testing.T, coord *Coordinator, chainID string, valSet *octypes.ValidatorSet, signers map[string]octypes.PrivValidator, opts ...wasm.Option) *TestChain {
...
func (chain *TestChain) CreateTMClientHeader(chainID string, blockHeight int64, trustedHeight clienttypes.Height, timestamp time.Time, tmValSet, nextVals, tmTrustedVals *octypes.ValidatorSet, signers map[string]octypes.PrivValidator) *ibctmtypes.Header {
	var (
	...
	signerArr := make([]octypes.PrivValidator, len(tmValSet.Validators))
	for i, v := range tmValSet.Validators {
		signerArr[i] = signers[v.Address.String()]
	}

	commit, err := octypes.MakeCommit(blockID, blockHeight, 1, voteSet, signerArr, timestamp)
	require.NoError(chain.t, err)
