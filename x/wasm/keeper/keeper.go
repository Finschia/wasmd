...
func (k Keeper) CallCallablePoint(ctx sdk.Context, contractAddress sdk.AccAddress, argsEv []byte, callablePointName string, isReadonly bool) ([]byte, error) {
	defer func(begin time.Time) { k.metrics.ExecuteElapsedTimes.Observe(time.Since(begin).Seconds()) }(time.Now())
	contractInfo, codeInfo, prefixStore, err := k.contractInstance(ctx, contractAddress)
	if err != nil {
		return nil, err
	...
	emptyBin, err := json.Marshal([]wasmvmtypes.HumanAddress{})
	if err != nil {
		panic(fmt.Sprintf("failed to marshal JSON: %s", err))
	}
	data, evts, attrs, gasUsed, execErr := k.wasmVM.CallCallablePoint(callablePointNameBin, codeInfo.CodeHash, isReadonly, emptyBin, env, argsEv, wasmStore, k.cosmwasmAPI(ctx), querier, k.gasMeter(ctx), gas, costJSONDeserialization)
